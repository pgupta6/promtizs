...
function DimensionBar({ label, score, max, feedback }: { label: string; score: number; max: number; feedback: string }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      >
        <span className="text-gray-700 dark:text-gray-300 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">{label}</span>
      ...
      {expanded && (
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 pl-1">{feedback}</p>
      )}
    </div>
  );
}
      ...
      <div className="flex flex-col items-center gap-4 py-2">
        <div className="relative">
          <svg className="w-32 h-32 -rotate-90" viewBox="0 0 128 128">
            <circle cx="64" cy="64" r="56" fill="none" stroke="var(--score-ring-bg)" strokeWidth="8" />
            <defs>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 max-w-md text-center">{result.summary}</p>
      </div>

      {/* Dimension Breakdown */}
      <div className="glass rounded-2xl p-6">
        <h3 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-5">Score Breakdown</h3>
      ...
      <div className="glass rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between p-5 border-b border-gray-200 dark:border-white/[0.06]">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Improved Prompt</h3>
          </div>
          <button
            onClick={copyImproved}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs glass-subtle hover:bg-gray-100 dark:hover:bg-white/[0.06] text-gray-600 dark:text-gray-300 rounded-lg transition-all"
          >
        ...
        <div className="p-5">
          <p className="text-slate-800 dark:text-gray-200 text-sm leading-relaxed whitespace-pre-wrap font-mono">
            ...
            <div className="p-5 pt-0 space-y-3">
              {result.changes.map((change, i) => (
                <div key={i} className="flex gap-3 text-sm">
                  ...
                  <div>
                    <span className="text-slate-800 dark:text-gray-200 font-medium">{change.what}</span>
